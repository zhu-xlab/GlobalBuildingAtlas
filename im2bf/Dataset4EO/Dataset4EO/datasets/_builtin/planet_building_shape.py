...
class PlanetBuildingShape(Dataset):
    """
    ...
    def _datapipe(self, resource_dp):
        ...
        shp_dp, _ = Demultiplexer(resource_dp[0], 2, self._classify_dp)
        shp_paths = list(shp_dp)
        ...
        for shp_path in shp_paths:
            ...
            geometries = []
            feature_collection = fiona.open(shp_path)
            print(f'loading annotations from {shp_path}, sizes: {len(feature_collection)}')
            for feature in feature_collection:
                ...
                if geometry is not None and geometry.geom_type == 'Polygon' and geometry.is_valid:
                    # cur_bound = np.array(geometry.bounds).reshape(2, 2)
                    geometries.append(geometry)

            # lens = [g.length for g in geometries]
            geom_dict[pathlib.Path(shp_path).name] = geometries
    ...
    def __len__(self) -> int:
        ...
        lens = [len(g) for k, g in self.geom_dict.items()]
        return sum(lens)
