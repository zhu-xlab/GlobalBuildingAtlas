...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', DtImgIds=None, gt_bounds=None,
                 dt_bounds=None):
        '''
        ...
        self.params.maxDets = [10000, 10000, 10000]
        self._paramsEval = {}  # parameters for evaluation
        self.stats = []  # result summarization
        self.ious = {}  # ious between all gts and dts
        self.gt_bounds = gt_bounds
    ...
    def get_within_bounds_ids(self, xywh, bounds, type='has_intersection'):
        """
        ...
        if len(ious):
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    iou = min([t, 1 - 1e-10])
                    m = -1
                    if self.gt_bounds is not None:
                        valid_inds = self.get_within_bounds_ids(d['bbox'], self.gt_bounds[imgId])
                        valid_inds = valid_inds.nonzero()[0]
                    else:
                        valid_inds = range(len(gt))
                    # for gind, g in enumerate(gt):
                    for gind in valid_inds:
                        g = gt[gind]
    ...
    def summarize(self):
        ...
        def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=100):
            ...
            result_str = iStr.format(titleStr, typeStr, iouStr, areaRng, maxDets, mean_s)
            self.result_str.append(result_str)
            print(result_str)
            return mean_s
