...
class InriaPolygonizedDataset(EODataset):

    def __init__(self, coco_ann_path=None, gt_seg_map_loader_pipeline=None, **kwargs):
        super(InriaPolygonizedDataset, self).__init__(**kwargs)
    ...
    def polygons2annotations(self, polygons, img_id, ann_start_id):
        annotations = []
        bounds = []
        for i, poly in enumerate(polygons):
            ...
            annotation = {
                ...
                "category_id": 1,  # Assuming all polygons belong to one category
    ...
    def get_coco_json(self, img_metas, polygons):
        ann_start_id = 1
        ann_infos = []
        img_infos = []
        categories = [
        ...
        bounds = {}
        for img_id, (img_meta, cur_polygons) in enumerate(zip(img_metas, polygons)):
            filename = img_meta['filename']
            cur_anns, cur_bounds = self.polygons2annotations(cur_polygons, img_id + 1, ann_start_id)
            bounds[img_id + 1] = cur_bounds
    ...
    def evaluate(self,
        ...
        if 'continent_name' in results[0][1]:
        ...
        if 'polygons' in results[0][0]:
            ...
            coco_dt_str, dt_bounds = self.get_coco_json(img_metas, dt_polygons)
            ...
            coco_eval = COCOeval(coco_gt, coco_dt, DtImgIds=coco_dt.getImgIds(),
                                 gt_bounds=gt_bounds, dt_bounds=dt_bounds)
            coco_eval.evaluate()
        ...
        if 'pre_iou' in results[0][0]:
            ...
            for city_name in city_names:
                ...
                for key, value in ret_metrics_summary.items():
                    if key == 'aAcc':
                        cur_iou_eval_result[key] = value / 100.0
                    else:
                        cur_iou_eval_result['m' + key] = value / 100.0

                ret_metrics_class.pop('Class', None)
                for key, value in ret_metrics_class.items():
                    cur_iou_eval_result.update({
                        key + '.' + str(name): value[idx] / 100.0
                        for idx, name in enumerate(class_names)
                    })

                city_iou[city_name] = ret_metrics['IoU'][1]
                iou_eval_results[city_name] = cur_iou_eval_result

            eval_results['IoU Related'] = iou_eval_results
            all_values = []
