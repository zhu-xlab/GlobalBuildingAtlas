...
class NMSPointSampler(metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.num_max_gt = num_max_gt
        self.nms_width = nms_width
    ...
    def _sample_pos(self, assign_result, num_expected, points, **kwargs):
        ...
        if num_expected < 0 or pos_inds.numel() <= num_expected:
            return pos_inds
        else:
            return self.random_choice(pos_inds, num_expected)

    def _sample_neg(self, assign_result, num_expected, points, **kwargs):
        """Randomly sample some negative samples."""
        neg_inds = torch.nonzero(assign_result.gt_inds == 0, as_tuple=False)
        # if neg_inds.numel() != 0:
        neg_inds = neg_inds.squeeze(1)
        if num_expected > 0 and len(neg_inds) <= num_expected:
            return neg_inds
        else:
            start = points[neg_inds] - self.nms_width / 2.
            end = points[neg_inds] + self.nms_width / 2.
            bboxes = torch.cat([start, end], dim=1)
            probs = torch.rand(bboxes.shape[0], device=points.device)
            keep_idx = mmcv.ops.nms(bboxes, probs, iou_threshold=0.0)[1].cpu().numpy()

            return neg_inds[keep_idx]
