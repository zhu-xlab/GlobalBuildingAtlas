...
class MMSegWandbHook(WandbLoggerHook):
    ...
    def before_run(self, runner):
        ...
        for hook in runner.hooks:
            ...
            if isinstance(hook, EvalHook):
                from rsipoly.apis import single_gpu_test
                self.eval_hook = hook
                self.test_fn = single_gpu_test
            if isinstance(hook, DistEvalHook):
                from rsipoly.apis import multi_gpu_test
                self.eval_hook = hook
    ...
    def _add_ground_truth(self, runner):
        ...
        from rsipoly.datasets.pipelines import LoadImageFromFile
        img_loader = None
        ...
        for idx in self.eval_image_indexs:
            ...
            img_meta = img_loader(
                dict(img_info=img_info, img_prefix=self.val_dataset.data_root))
