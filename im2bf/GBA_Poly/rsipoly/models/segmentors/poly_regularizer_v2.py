...
class PolyRegularizerV2(EncoderDecoder):
    ...
    def __init__(
        ...
        super(PolyRegularizerV2, self).__init__(backbone, decode_head, **kwargs)
        ...
        for features in batch_features:
            ...
            for temp in features:
                for feat in temp:
                    ...
                    # exterior = (np.array(feat['exterior']) / downscale - 0.5).tolist()
                    # interiors = [(np.array(x) / downscale - 0.5).tolist() for x in feat['interiors']]
    ...
    def forward_train(self, ori_img, img_metas, gt_semantic_seg, contours, contour_labels, gdal_features, gt_features, **kwargs):
        ...
        raster_input = self.rasterize(gt_features, add_noise=True, offset=0.0)
    ...
    def slide_inference(self, img, img_meta, rescale, contours=None, contour_labels=None,
        ...
        num_classes = 2
        preds = img.new_zeros((batch_size, num_classes, h_img * out_scale_factor, w_img * out_scale_factor))
        count_mat = img.new_zeros((batch_size, 1, h_img * out_scale_factor, w_img * out_scale_factor))
        ...
        for i in range(batch_size):
            ...
            for crop_idx, crop_box in enumerate(tqdm(crop_boxes)):
                ...
                if self.test_cfg.get('upscale_output', True):
                ...
                preds[i:i+1, :, start_y * out_scale_factor:end_y * out_scale_factor,
                      start_x * out_scale_factor:end_x * out_scale_factor] += crop_seg_logit
