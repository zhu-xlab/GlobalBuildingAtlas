...
class PolySimplifierV2(EncoderDecoder):
    ...
    def __init__(
        ...
        window_feat_size=6, len_sampled_segments=50, noise_conf=None,
        dice_loss_conf=None, **kwargs,
    ):
        super(PolySimplifierV2, self).__init__(backbone, decode_head, **kwargs)
    ...
    def polygonize(self, img):
        ...
        polygons = []
        rings = []
        for i, batch_img in enumerate(img):
            cur_shapes = shapes(batch_img[0].cpu().numpy(), mask=batch_img[0].cpu().numpy() > 0)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, contours, contour_labels, gdal_features, gt_features, **kwargs):
        ...
        """
        gdal_features = gdal_features[0]['features']
        gt_features = gt_features[0]['features']
        ...
        polygons, rings = self.polygonize(img)
