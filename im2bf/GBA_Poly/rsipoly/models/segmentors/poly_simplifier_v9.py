...
class PolySimplifierV9(EncoderDecoder):
    ...
    def __init__(
        ...
        super(PolySimplifierV9, self).__init__(backbone, decode_head, **kwargs)
        ...
        for i, gt_polygons in enumerate(batch_gt_polygons):
            ...
            for polygon in gt_polygons:
                ...
                if input_poly_type == 'polygonized':
                    ...
                    ring = self.polygon_processor.sample_points_in_ring(norm_exterior)
                    if ring is None:
                        continue
    ...
    def forward_train(self, gt_features, **kwargs):
        ...
        ring_angle_targets = torch.cat(ring_angle_targets, dim=0).to(device)
        ...
        """
        temp = torch.zeros(N, int(max_sample_length), int(max_sample_length), 32).to(device)
        pos_encoding = self.pos_enc(temp).permute(0,3,1,2)
        point_feats = F.grid_sample(
            pos_encoding,
        ...
        loss_ring_angle = self.l1_loss_fun(
            ring_pred_angle.view(-1), ring_angle_targets.view(-1)
        )
        mask = rings >= 0


        if 'ring_angle' in self.loss_weights and self.loss_weights['ring_angle'] > 0:
    ...
    # TODO refactor
    def slide_inference(self, img, img_meta, rescale=None, contours=None, contour_labels=None,
        ...
        all_rings, all_idxes, all_ring_sizes = self.polygon_processor.sample_rings(polygons)
        ...
        for i in tqdm(range(num_iter), desc=f'Simplifying the polygons on {file_str}/{box_str}'):
        ...
        self.polygon_processor.post_process(pred_rings, ring_pred_next, all_idxes, all_ring_sizes, img_meta[0])
