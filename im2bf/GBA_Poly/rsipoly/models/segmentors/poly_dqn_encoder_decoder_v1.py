...
class PolyDQNEncoderDecoderV1(EncoderDecoder):
    ...
    def __init__(self, backbone, decode_head, assigner, sampler, dqn_head, point_head, wandb_cfg=None,
                 ...
                 dqn_skill_level=4, matching_head=None, **kwargs):
        super(PolyDQNEncoderDecoderV1, self).__init__(backbone, decode_head, **kwargs)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if not self.freeze_base_net:
            optimizer['base_net'].zero_grad()
            losses, states, _ = self(**data_batch)
            loss, log_vars = self._parse_losses(losses)
            loss.backward()
            optimizer['base_net'].step()
        else:
            with torch.no_grad():
                dqn_states = self(**data_batch, for_dqn=True)

            self.dqn_head.set_environment(**dqn_states, cur_iter=0)
            init_success, game_level = self.dqn_head.init_game(self.dqn_skill_level)
            if init_success:
                for i in range(self.num_dqn_update_per_step):
                    # if i <= game_level * 10:
                    if i <= self.num_dqn_update_per_step // 2:
                        # first take an exam
                        optimizer['dqn_head'].zero_grad()
                        losses_dqn, states = self.dqn_head(exam_mode=True, game_level=game_level)
                        loss, log_vars = self._parse_losses(losses_dqn)
                        loss.backward()
                        optimizer['dqn_head'].step()

                        if i <= game_level * 2 and states.get('pass_exam', False):
                            if not game_level in self.num_game_success.keys():
                                self.num_game_success[game_level] = 0
                            # print(f'pass: {game_level} {self.num_game_success[game_level]}')
                            self.num_game_success[game_level] += 1
                            if (self.num_game_success[game_level] >= 5) \
                               & (game_level + 1 > self.dqn_skill_level):
                                print(f'dqn skill level up to: {game_level+1}!')
                                self.dqn_skill_level = game_level + 1
                            break
                        if i <= game_level * 10 and states.get('pass_exam', False):
                            break
                    else:
                        optimizer['dqn_head'].zero_grad()
                        losses_dqn, states = self.dqn_head(game_level=game_level)
                        loss, log_vars = self._parse_losses(losses_dqn)
                        loss.backward()
                        optimizer['dqn_head'].step()

                    if self.dqn_head.cnt_valid * game_level / self.num_dqn_update_per_step >= 0.2:
                        break
    ...
    def forward_train_encode_decode(self, img, img_metas, contours, contour_labels, gt_points,
        ...
        # contours = torch.cat([contours, gt_points])
        ...
        x = self.extract_feat(crop_imgs)
        loss_decode, state_decode = self._decode_head_forward_train(x, img_metas, crop_masks)
        losses.update(loss_decode)
        states.update(state_decode)
        feats = state_decode['feats']

        crop_points_list = []
        crop_point_feats_list = []
        ...
        for crop_idx, crop_box in enumerate(crop_boxes):
            ...
            if len(contours) > 0:
                ...
                crop_point_feats = F.grid_sample(
                    feats[crop_idx:crop_idx+1],
                    norm_crop_points.view(1, 1, -1, 2),
                    align_corners=True
                ).squeeze(2)[0].permute(1,0) # (num_points, num_channels)

                crop_points_list.append(crop_points + offset)
                crop_point_feats_list.append(crop_point_feats)
        ...
        all_point_feats = torch.cat(crop_point_feats_list, dim=0)
        ...
        loss_point_cls, all_pred_point_cls = self.point_head(
            point_feats=all_point_feats,
            gt_inds=all_gt_inds,
            mode='point_cls'
        )
        losses.update(loss_point_cls)

        loss_angle, all_pred_degrees = self.point_head(
            points=all_points,
            point_feats=all_point_feats,
            gt_angles=all_gt_degrees,
            mode='angle'
        )
        losses.update(loss_angle)
        ...
        for comp_idx in comp_idxes:
            ...
            if len(cur_inds) > 0 and ~(next_inds == -1).any() and \
               ...
               (cur_inds.unique() == next_inds.unique()).all(): # valid components
                ...
                comp_normalized_points.append(self.normalize_comp_points(all_points[comp_mask].float(),
                                                                         method=self.comp_norm_mode))
                comp_pred_degrees.append(all_pred_degrees[comp_mask])
                comp_pred_point_cls.append(all_pred_point_cls[comp_mask])

                if self.use_point_feats:
                    comp_point_feats.append(all_point_feats[comp_mask])
        ...
        sizes = [len(points) for points in comp_points]
        if len(sizes) == 0:
            return losses, states
    ...
    def process_patch(self, img, contours, contour_labels, offset, scale_factor):
        ...
        norm_proposals = self.matching_head.normalize_coordinates(proposals, W, 'global')
    ...
    def check_and_dump_polygons(self):
        ...
        if sum(sizes) > 2048:
            ...
            for i, batch_idxes in enumerate(batch_idx_list):
                ...
                point_preds[i, :len(batch_point_preds)] = \
                        self.normalize_comp_points(batch_point_preds, method=self.comp_norm_mode)
                cur_idx = 0
