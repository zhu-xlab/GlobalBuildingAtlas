...
class PolySimplifierV15(BaseModule):
    ...
    def __init__(
        ...
        super(PolySimplifierV15, self).__init__()
    ...
    def sample_rings(self, pred_polygon, gt_polygon, k_cls_target=2):
        ...
        # sampled_angles = polygon_utils.calculate_angle_between_points(sampled_pred_points)
        # projected_angles = polygon_utils.calculate_angle_between_points(projected_pred_points)
        sampled_angles = sampled_pred_points[1:] - sampled_pred_points[:-1]
        projected_angles = projected_pred_points[1:] - projected_pred_points[:-1]

        gt_points = np.array(gt_polygon.exterior.coords)
        # gt_segments = np.concatenate([gt_points[:-1], gt_points[1:]], axis=-1)
        # projected_pred_points_2 = self.project_points_onto_segments(gt_segments, sampled_pred_points)


        squ_dis = ((np.expand_dims(projected_pred_points, 0) - np.expand_dims(gt_points, 1)) ** 2).sum(axis=-1)
        # matched_idxes = squ_dis.argmin(axis=1)
        ...
        angle = torch.from_numpy(sampled_angles).float()
        ...
        shuffled_angle = torch.cat([angle[start_idx:], angle[:start_idx]])
        ...
        sampled_angle = torch.zeros(max_ring_len - 1, 2, dtype=shuffled_angle.dtype) - 1
        sampled_angle_target = torch.zeros(max_ring_len - 1, 2, dtype=shuffled_angle_target.dtype) - 1
        ...
        sampled_angle[:len(shuffled_angle)] = shuffled_angle[:max_ring_len-1]
        sampled_angle_target[:len(shuffled_angle)] = shuffled_angle_target[:max_ring_len-1]

        return sampled_ring, sampled_cls_target, sampled_reg_target, sampled_angle, sampled_angle_target
    ...
    def cal_loss_angle(self, pred, target, mask, cls_target=None, type='all_points'):
        ...
        def get_angles(points):
            # ext_polygon = torch.cat([points, points[:, 0:1]], dim=1)
            diff = points[:,1:] - points[:,:-1]
            degs = torch.fmod(torch.arctan2(- diff[:,:,1], diff[:,:,0]), 2 * np.pi)

            return degs / np.pi

        def get_angles2(points):
            ...
            # Calculate the angle between the vectors
            cosine_angle = dot_product / (magnitude_v1 * magnitude_v2 + 1e-9)
            cosine_angle = torch.clip(cosine_angle, -1, 1)
            angle_radians = torch.acos(cosine_angle)
        ...
        if type == 'all_points':
            pred_angles = get_angles(pred)
            target_angles = get_angles(target)
            valid_mask = get_segments(mask).view(B, N-1, -1).all(dim=-1)

            loss_d1 = self.l1_loss_fun(pred_angles[valid_mask], target_angles[valid_mask]).mean()

            pred_angles2 = get_angles2(pred)
            target_angles2 = get_angles2(target)
            valid_mask2 = get_two_segments(mask).view(B, N-2, -1).all(dim=-1)

            loss_d2 = self.l1_loss_fun(pred_angles2[valid_mask2], target_angles2[valid_mask2]).mean()

            return {'loss_deg1': loss_d1 * self.loss_weights['ring_angle'], 'loss_deg2': loss_d2 * self.loss_weights['ring_angle']}, {}
            ...
            for i in range(B):
                ...
                cur_pred_segments = get_two_segments(cur_pred.unsqueeze(0))
                cur_target_segments = get_two_segments(cur_target.unsqueeze(0))
    ...
    def forward_train(self, img, seg_logits, gt_semantic_seg, gt_features, **kwargs):
        ...
        matched_rate = sum(num_matched) / sum(num_pred)
        ...
        use_prob_window = self.ring_sample_conf.get('use_prob_window', False)

        rings, offset_rings, norm_rings, ring_cls_targets, ring_reg_targets, ring_probs = [], [], [], [], [], []
        ring_angles, ring_angle_targets = [], []
        for i, matched_rings in enumerate(batch_matched_rings):
            cur_rings, cur_ring_cls_targets, cur_ring_reg_targets = [], [], []
            cur_ring_angles, cur_ring_angle_targets = [], []
            for pred_ring, gt_ring in matched_rings:
                ring, ring_cls_target, ring_reg_target, ring_angle, ring_angle_target = self.sample_rings(pred_ring, gt_ring)

                cur_rings.append(ring)
                cur_ring_cls_targets.append(ring_cls_target)
                cur_ring_reg_targets.append(ring_reg_target)
                cur_ring_angles.append(ring_angle)
            ...
            if len(cur_rings) > 0:
                ...
                ring_prob = self.get_ring_probs(norm_ring, probs[i:i+1], use_prob_window)
                ...
                ring_angles.extend([x for x in cur_ring_angles])
        ...
        if len(rings) > max_num_rings:
            ...
            ring_angles = [ring_angles[idx] for idx in rand_idxes]
        ...
        # ring_angles = torch.stack(ring_angles).to(img.device)
        ring_angle_targets = torch.stack(ring_angle_targets).to(img.device)

        N, L, _ = rings.shape
        seq = (torch.arange(L) / (L - 1) * 2 - 1).view(1, -1, 1).repeat(N,1,1).to(img.device)
    ...
    def slide_inference(self, img, mask, probs, img_meta, rescale=None, contours=None, contour_labels=None,
        ...
        poly_json = self.polygonize(mask[:,0])[0]
        ...
        ring_probs = self.get_ring_probs(norm_rings, probs, use_prob_window=use_prob_window)
        rings_input = torch.cat([norm_rings, ring_probs, seq], dim=-1)
        ...
        pred_polygons = self.polygon_processor.post_process_by_cls(pred_rings, ring_pred_cls, all_idxes, all_ring_sizes)
        ...
        result['vis|polygon_img_pred'] = [img, [pred_polygons_shape]]
    ...
    def get_ring_probs(self, norm_ring, probs, use_prob_window=False):
        num_rings, ring_len, _ = norm_ring.shape
        B, C, H, W = probs.shape
        assert B == 1

        if not use_prob_window:
            feats = probs
        else:
            feats = self.unfold_fun(probs[:,1:]).view(B, -1, H, W)

        ring_prob = F.grid_sample(
            feats, norm_ring.view(1, 1, num_rings * ring_len, 2).to(probs.device),
            align_corners=True
        ).squeeze(2).permute(0,2,1) # (B, num_points, num_channels)
