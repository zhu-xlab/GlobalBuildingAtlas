...
class PolyEncoderDecoderV9(EncoderDecoder):
    ...
    def __init__(self, backbone, decode_head, assigner, sampler, matching_head, wandb_cfg=None,
                 comp_norm_mode='standard', use_point_feats=False, filter_separated_comp=False, **kwargs):
        super(PolyEncoderDecoderV9, self).__init__(backbone, decode_head, **kwargs)
    ...
    def forward_train(self, img, img_metas, contours, contour_labels, gt_points, gt_degrees, polygons, mask, **kwargs):
        ...
        for crop_idx, crop_box in enumerate(crop_boxes):
            ...
            if len(contours) > 0:
                ...
                norm_crop_points = self.matching_head.normalize_coordinates(crop_points, w_crop, 'global')
        ...
        loss_point_cls = self.matching_head(
        ...
        loss_angle, all_pred_degrees = self.matching_head(
            points=all_points,
            point_feats=all_point_feats,
        ...
        loss_matching, state_matching = self.matching_head(
            point_feats=batch_pred_degrees if not self.use_point_feats else batch_point_feats,
    ...
    def slide_inference(self, img, img_meta, rescale, contours=None, contour_labels=None,
                        comp_mask=None, scale_factor=4, **kwargs):
    ...
    def process_patch(self, img, contours, contour_labels, offset, scale_factor):
        ...
        if len(keep_idx) > 0:
            ...
            pos_point_feats = pred_degrees[0, keep_idx]
    ...
    def normalize_comp_points(self, points, buffer_ratio=0.2, max_w=256, max_h=256, method='standard'):
        ...
        if method == 'standard':
            ...
            new_points = self.matching_head.normalize_coordinates(points, a, 'global')
