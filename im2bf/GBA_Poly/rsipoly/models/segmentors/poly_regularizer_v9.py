...
class PolyRegularizerV9(EncoderDecoder):
    ...
    def __init__(
        self, backbone, decode_head, eroded_net, wandb_cfg=None, **kwargs,
    ):
        super(PolyRegularizerV9, self).__init__(backbone, decode_head, **kwargs)
        self.backbone = builder.build_backbone(backbone)
        # self.simp_net = builder.build_backbone(backbone)

        self.eroded_net = nn.Sequential(
            nn.BatchNorm2d(eroded_net['in_channels']),
            nn.ReLU(inplace=True),
            nn.Conv2d(eroded_net['in_channels'], 2, kernel_size=1, stride=1, padding=0, bias=True),
        )
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, eroded_gt_semantic_seg=None):
        ...
        seg_logits, state = self.decode_head.forward(x)
        feats = state['feats']

        gt_semantic_seg = (gt_semantic_seg > 0).long()
        losses_seg = self.decode_head.losses(seg_logits, gt_semantic_seg)
        losses.update(losses_seg)

        if eroded_gt_semantic_seg is not None:
            seg_logits_eroded = self.eroded_net(feats)
            eroded_gt_semantic_seg = (eroded_gt_semantic_seg > 0).long().unsqueeze(1)
            losses_eroded_seg = self.decode_head.losses(seg_logits_eroded, eroded_gt_semantic_seg)

            for key, value in losses_eroded_seg.items():
                losses[key + '_eroded'] = value
        ...
        if eroded_gt_semantic_seg is not None:
            states['seg_logits_eroded'] = seg_logits_eroded
            states['eroded_gt'] = eroded_gt_semantic_seg

        return losses, states

    def assign_gt_pred(self, pred_mask, gt):

        idx_pred = pred_mask.cpu().nonzero()
        idx_gt = gt.cpu().nonzero()
    ...
    def slide_inference(self, img, img_meta, rescale=None, **kwargs):
        ...
        if 'geo_transform' in img_meta[0] and img_meta[0]['geo_transform'] is None:
            return {}
        ...
        if seed_type == 'erode':
            preds_eroded = torch.zeros(batch_size, num_classes, out_h, out_w)
            count_mat_eroded = torch.zeros(batch_size, num_classes, out_h, out_w)
        ...
        for i in range(batch_size):
            ...
            # for j in range(len(splits) - 1):
                ...
                if seed_type == 'erode':
                    crop_seg_logit_eroded = self.eroded_net(feats)
                    crop_seg_logit_eroded = F.interpolate(crop_seg_logit_eroded, size=(out_crop_h, out_crop_w), mode='bilinear')

                for crop_idx, crop_box in enumerate(selected_crop_boxes[splits[j]:splits[j+1]]):

                    if out_size_scale is not None:
                        start_x, start_y, end_x, end_y = (crop_box // (h_img  / out_h * out_size_scale)).astype(np.int)
                    else:
                    ...
                    if seed_type == 'erode':
                        temp2 = crop_seg_logit_eroded[crop_idx:crop_idx+1].cpu() * weights.view(1,1, out_crop_h, out_crop_w)
                        preds_eroded[i:i+1, :, start_y:end_y, start_x:end_x] += temp2
                        count_mat_eroded[i:i+1, :, start_y:end_y, start_x:end_x] += 1
        ...
        if seed_type == 'erode':
            preds_eroded = preds_eroded / (count_mat_eroded + 1e-8)
            pred_masks_eroded = preds_eroded.max(dim=1)[1]
            color_masks_eroded = self.find_connected_components(pred_masks_eroded.cpu().numpy())
            color_masks_eroded = torch.tensor(np.array(color_masks_eroded))

            output = polygon_utils.map_nearest_nonzero(pred_masks[0].cpu().numpy(), color_masks_eroded[0].cpu().numpy())
            # color_masks = self.assign_gt_pred(pred_masks, color_masks_eroded)
            output = self.find_connected_components([output])
            output = torch.from_numpy(np.array(output)).int()

        elif seed_type == 'normal':
            prob_thre = self.test_cfg.get('prob_thre', 0.3)
            output = (probs[:,1] > prob_thre).to(torch.uint8)
            output = self.find_connected_components(output)
            output = torch.from_numpy(np.array(output)).int()

        elif seed_type == 'double_thre':
            l_thre, r_thre = self.test_cfg.get('prob_thre', [0.3, 0.5])
            seed = (probs[:,1] > r_thre).cpu().numpy()
            seed = self.find_connected_components(seed)[0]
            mask = (probs[:,1] > l_thre).cpu().numpy()[0]
            output = polygon_utils.map_nearest_nonzero(mask, seed)
            output = self.find_connected_components([output])
            output = torch.from_numpy(np.array(output)).int()
        ...
        # output = preds.max(dim=1)[1].int()
        # return output, preds.max(dim=1)[1], probs
        return dict(pred_mask=output, prob_map=probs)

    # Function to find connected components
    def find_connected_components(self, imgs):
        components = []
        for img in imgs:  # Iterate over batch
            component = label(img, connectivity=1)  # 4-connectivity
