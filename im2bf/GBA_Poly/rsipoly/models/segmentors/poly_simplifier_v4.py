...
class PolySimplifierV4(EncoderDecoder):
    ...
    def __init__(
        self, backbone, decode_head, simp_net, assigner, sampler, wandb_cfg=None,
        window_feat_size=6, len_sampled_segments=50, noise_conf=None, linear_conf=None,
        dice_loss_conf=None, **kwargs,
    ):
        super(PolySimplifierV4, self).__init__(backbone, decode_head, **kwargs)
    ...
    def polygonize(self, img):

        cur_shapes = shapes(img[0].cpu().numpy(), mask=img[0].cpu().numpy() > 0)

        cur_polygons = []
        cur_rings = []
        for shape, _ in cur_shapes:
            cur_polygons.append(shapely.geometry.shape(shape))

            for x in shape['coordinates']:
                cur_rings.append(torch.tensor(x, device=img.device))

        return cur_polygons, cur_rings
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, contours, contour_labels, gdal_features, gt_features, **kwargs):
        ...
        raster_gt = np.array(self.rasterize(gt_features))
        raster_gt = torch.tensor(raster_gt, device=img.device).float()
        raster_gt = F.interpolate(raster_gt.unsqueeze(1), size=(H, W)).long()

        img = (raster_gt > 0).float() - 0.5
        ...
        points, pixel2grid = self.sample_grids(H, W, device=img.device)
        N = points.shape[0]
        points_np = points.cpu().numpy()
        kd_tree = cKDTree(points_np)
        _, graph = kd_tree.query(points_np, K + 1)
        graph = torch.tensor(graph, device=img.device)
        ...
        # ori_polygons, ori_rings, ori_ring_idxes = self.polygonize(img[0])
        # noise_polygons, noise_rings, noise_ring_idxes = self.polygonize(img[0])

        # states['gdal_polygons'] = rings
