...
class PolygonizerV9(BaseModule):
    ...
    def __init__(
        ...
        init_cfg=None, overide_cfg=None, wandb_cfg=None, **kwargs,
    ):
        super(PolygonizerV9, self).__init__(init_cfg)
        self.regu_net = builder.build_segmentor(regu_net)
        self.simp_net = builder.build_segmentor(simp_net)
        self.test_cfg = test_cfg
        self.train_cfg = train_cfg
        self.wandb_cfg = wandb_cfg
        self.overide_cfg = overide_cfg
    ...
    def init_weights(self, init_cfg=None):
        ...
        def modify_state(state_dict, prefix):
            new_state_dict = state_dict.copy()
            for key, value in state_dict.items():
                new_state_dict[key[len(prefix)+1:]] = value
        ...
        for key in ['regu_checkpoint', 'simp_checkpoint']:
            if key in init_cfg:
                prefix = None
                if type(init_cfg[key]) == str:
                    checkpoint = init_cfg[key]
                else:
                    prefix = init_cfg[key].get('prefix', None)
                    checkpoint = init_cfg[key]['path']
                checkpoint = torch.load(checkpoint, map_location='cpu')['state_dict']
                if prefix is not None:
                    checkpoint = modify_state(checkpoint, prefix)

                net = self.regu_net if key.startswith('regu') else self.simp_net
                log1 = net.load_state_dict(checkpoint, strict=False)
    ...
    def forward_train(self, **kwargs):
        ...
        train_type = self.train_cfg.get('train_type', 'regu_net')
        img_metas = kwargs['img_metas']
        losses, states = {}, {}

        if train_type == 'simp_net':
            losses_simp, states_simp = self.simp_net.forward_train(kwargs['features'])
            losses.update(losses_simp)
            states.update(states_simp)

        elif train_type == 'combined':
        ...
        elif train_type == 'regu_net':
            img = kwargs['img']
            gt_semantic_seg = kwargs['gt_semantic_seg']
            eroded_gt_semantic_seg = kwargs.get('eroded_gt_semantic_seg', None)
    ...
    def slide_inference(self, img, img_meta, rescale, **kwargs):
        ...
        skip_simp = self.test_cfg.get('skip_simp', False)
        if self.overide_cfg is not None and not hasattr(self, 'overide_flag'):
            self.init_weights(self.overide_cfg)
            self.overide_flag = 1
        ...
        result_regu = self.regu_net.slide_inference(img, img_meta)
        result.update(result_regu)
        if result_regu == {}:
            return result

        pred_mask = result_regu['pred_mask']
        prob_map = result_regu['prob_map']

        if not skip_simp:
            result_simp = self.simp_net.slide_inference(img, pred_mask.unsqueeze(0), prob_map, img_meta, rescale=rescale)
            result.update(result_simp)

        # shape_polygons = [p.simplify(tolerance=2, preserve_topology=True) for p in shape_polygons]

        result['img'] = img
        result['prob_map'] = prob_map
        result['pred_mask'] = (pred_mask[0].cpu() > 0).to(torch.uint8).numpy()
        result['vis|featmap'] = [img, prob_map[:,0:1]]
