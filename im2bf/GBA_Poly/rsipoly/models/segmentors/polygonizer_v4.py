...
class PolygonizerV4(BaseModule):
    ...
    def __init__(
        ...
        super(PolygonizerV4, self).__init__(init_cfg)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if 'img' in states:
            img = states['img']
            gt_semantic_seg = (states['gt_semantic_seg'] > 0).to(torch.uint8)
            states['vis|masks_img_gt'] = [img, gt_semantic_seg[:,0].cpu().numpy()]
    ...
    def forward_train(self, img, gt_semantic_seg, **kwargs):
        ...
        losses_regu, states_regu = self.regu_net.forward_train(
            img, img_metas, (gt_semantic_seg > 0).to(torch.uint8)
        )
    ...
    def slide_inference(self, img, img_meta, rescale, **kwargs):
        ...
        reverse_pred = self.test_cfg.get('reverse_pred', False)
        eval_majority_voting = self.test_cfg.get('eval_majority_voting', False)

        if eval_majority_voting:
            assert 'majority_voting' in kwargs
            pred_mask = kwargs['majority_voting']
        ...
        if not eval_majority_voting:
            result['vis|featmap'] = [img, probs[:,0:1]]

        if 'gt_semantic_seg' in kwargs:
            gt = kwargs['gt_semantic_seg']
            result['vis|masks_img_gt'] = [img, (gt > 0).to(torch.uint8).cpu().numpy()]
            loss_cls = self.regu_net.cse_loss_fun(preds, (gt > 0).to(preds.device).long())
            result['loss'] = loss_cls

        result['vis|masks_img_pred'] = [img, pred_mask[0][None]]
