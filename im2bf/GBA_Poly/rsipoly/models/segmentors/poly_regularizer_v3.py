...
class PolyRegularizerV3(EncoderDecoder):
    ...
    def __init__(
        self, backbone, decode_head, simp_net, discriminator, gan_loss_conf, assigner, sampler, wandb_cfg=None,
        ...
        super(PolyRegularizerV3, self).__init__(backbone, decode_head, **kwargs)
        ...
        self.nms_width = 7
        self.cur_iter = 0
        self.disc_step = self.train_cfg.get('disc_step', 5)
        ...
        self.unfold_fun = nn.Unfold(kernel_size=3,
                                    padding=1,
                                    dilation=1)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if self.noise_conf:
            states['vis|masks_noise_img_gt'] = [states['noise_imgs'], states['raster_gt'][:, 0].cpu().numpy()]
    ...
    def sample_points_in_ring(self, ring, interval=None, add_noise=False):
        ...
        if add_noise:
            ...
            if noise_type == 'uniform':
                noise = np.random.rand(len(sampled_points), 2) * interval / 2.01
            elif noise_type == 'skip':
                noise = np.random.rand(len(sampled_points), 2) * interval / 1.01
                noise[0:2:-1] = 0
    ...
    def rasterize(self, batch_features, downscale=4, raster_shape=(256, 256), add_noise=False, filter_small=True):
        ...
        for features in batch_features:
            ...
            for temp in features:
                for feat in temp:
                    exterior = ((np.array(feat['exterior']) + downscale / 2) / downscale).tolist()
                    interiors = [((np.array(x) + downscale / 2) / downscale).tolist() for x in feat['interiors']]

                    if add_noise:
                        noise_exterior = self.sample_points_in_ring(exterior, add_noise=add_noise)
                        noise_interiors = [self.sample_points_in_ring(x, add_noise=add_noise) for x in interiors]

                        exterior = noise_exterior
                        interiors = noise_interiors

                        has_invalid = False
                        if len(noise_exterior) <= 3:
                            has_invalid = True
                        # for x in noise_interiors:
                        #     if len(x) <= 3:
                        #         has_invalid = True
                        if has_invalid:
                            continue
                    elif filter_small:
                        temp = self.sample_points_in_ring(exterior, add_noise=add_noise)
                        has_invalid = False
                        if len(temp) <= 3:
                            has_invalid = True
    ...
    def forward_train(self, ori_img, img_metas, gt_semantic_seg, contours, contour_labels, gdal_features, gt_features, **kwargs):
        ...
        raster_input = self.rasterize(gt_features, add_noise=True)
        raster_gt = self.rasterize(gt_features, raster_shape=(H, W), downscale=1)

        raster_input = torch.tensor(raster_input, device=ori_img.device).float()
        raster_gt = torch.tensor(raster_gt, device=ori_img.device).float()

        img = F.interpolate(raster_input.unsqueeze(1), size=(H, W)).int()
        img = (img > 0).float() - 0.5
        ...
        unfold_raster_gt = F.interpolate(raster_gt.unsqueeze(1), size=(H, W))
        unfold_raster_gt = self.unfold_fun(unfold_raster_gt).view(B, 9, H, W).long() # (B, C, k, H, W)
        cnt = (unfold_raster_gt > 0).sum(dim=1)
        sum = (unfold_raster_gt[:,4:5] == unfold_raster_gt).sum(dim=1)
        gt_semantic_seg = (unfold_raster_gt[:,4] > 0) & (cnt != sum)
        gt_semantic_seg = gt_semantic_seg.unsqueeze(1).long()
    ...
    def slide_inference(self, img, img_meta, rescale, contours=None, contour_labels=None,
        ...
        num_classes = 2
        preds = img.new_zeros((batch_size, num_classes, h_img * scale_factor, w_img * scale_factor))
        count_mat = img.new_zeros((batch_size, 1, h_img * scale_factor, w_img * scale_factor))
        ...
        for i in range(batch_size):
            ...
            for crop_idx, crop_box in enumerate(tqdm(crop_boxes)):
                ...
                crop_seg_logit, cur_state = self.encode_decode(crop_img, [img_meta[i]])

                """
                crop_seg_logit = F.pad(
                    crop_seg_logit,
                    (int(x1), int(preds.shape[3] - x2), int(y1), int(preds.shape[2] - y2))
                )
                """

                preds[i:i+1, :, start_y * scale_factor:end_y * scale_factor,
                      start_x * scale_factor:end_x * scale_factor] += crop_seg_logit

                count_mat[i:i+1, :, start_y * scale_factor:end_y * scale_factor,
                      start_x * scale_factor:end_x * scale_factor] += 1
