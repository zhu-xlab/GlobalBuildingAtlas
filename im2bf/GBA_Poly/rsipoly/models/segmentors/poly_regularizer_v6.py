...
class PolyRegularizerV6(EncoderDecoder):
    ...
    def __init__(
        ...
        super(PolyRegularizerV6, self).__init__(backbone, decode_head, **kwargs)
    ...
    def slide_inference(self, img, img_meta, rescale=None, **kwargs):
        ...
        num_classes = 2
        preds = torch.zeros(
            (batch_size, num_classes, int(h_img * out_scale_factor), int(w_img * out_scale_factor))
        )
        ...
        for i in range(batch_size):
            ...
            for j in range(len(splits) - 1):
                ...
                cur_img = F.interpolate(cur_img, scale_factor=(scale_factor, scale_factor), mode='nearest')
                ...
                # crop_seg_logit = F.interpolate(crop_seg_logit, scale_factor=(4, 4), mode='nearest')
                pdb.set_trace()
