...
class PolygonizerV3(BaseModule):
    ...
    def __init__(
        ...
        super(PolygonizerV3, self).__init__(init_cfg)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if 'img' in states:
            img = states['img']
            gt_semantic_seg = states['gt_semantic_seg']
    ...
    def forward_train(self, img, gt_semantic_seg, features, **kwargs):
        ...
        states['img'] = img
        states['gt_semantic_seg'] = gt_semantic_seg
    ...
    def slide_inference(self, img, img_meta, rescale, contours=None, contour_labels=None,
        ...
        pred_img = ((probs[:,1] > 0.5).float() - 0.5)
        pred_img = [pred_img[0].unsqueeze(0)]

        pred_polygons = self.simp_net.slide_inference(pred_img, img_meta, rescale=rescale)
        states['seg_logits'] = preds

        return pred_polygons, states
    ...
    def inference(self, img, img_meta, rescale, contours=None, contour_labels=None, **kwargs):
        ...
        if self.test_cfg.mode == 'slide':
            ...
            if 'seg_logits' in states:
                probs = F.softmax(states['seg_logits'], dim=1)[:, 1:, ...].detach()
                result['vis|featmap_probs'] = [img, probs]
