...
class PolySimplifierV5(EncoderDecoder):
    ...
    def __init__(
        ...
        ring_reg_net=None, ring_cls_net=None, dice_loss_conf=None, ring_sample_conf=None,
        loss_weights=None, **kwargs,
    ):
        super(PolySimplifierV5, self).__init__(backbone, decode_head, **kwargs)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if 'pred_rings' in states:
            states['vis|polygons_pred_rings'] = [data_batch['img'], states['pred_rings']]

        if 'gt_rings' in states:
            states['vis|polygons_gt_rings'] = [data_batch['img'], states['gt_rings']]

        if 'next_rings' in states:
            states['vis|polygons_next_rings'] = [data_batch['img'], states['next_rings']]
    ...
    def sample_points_in_ring(self, ring, interval=None):
        ...
        interval = self.ring_sample_conf['interval'] if interval is None else interval
        if self.train_cfg is not None and self.train_cfg.get('add_ring_noise', False):
            noise_type = self.train_cfg.get('noise_type', 'uniform')

            if noise_type == 'uniform':
                noise = np.random.rand(len(ring), 2) * interval / 2.01
            elif noise_type == 'vertical':
                pdb.set_trace()

            ring = (np.array(ring) + noise).tolist()
    ...
    def sample_rings(self, batch_polygons, batch_gt_polygons, ring_len=20):
        batch_rings = []
        batch_ring_cls_targets = []
        batch_ring_reg_targets = []

        for i, (polygons, gt_polygons) in enumerate(zip(batch_polygons, batch_gt_polygons)):
            rings = []
            ring_cls_targets = []
            ring_reg_targets = []
            for key in polygons.keys():
                ring, cls_target, reg_target = self.get_target_ring(polygons[key][0]['coordinates'][0], gt_polygons[key-1][0]['exterior'])
                if ring is None:
                    continue


                end_points = cls_target.nonzero().view(-1).tolist()

                # start_idx = random.choice(end_points)
                start_idx = random.randint(0, len(ring)-1)
        ...
        return batch_rings, batch_ring_cls_targets, batch_ring_reg_targets
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, contours, contour_labels, gdal_features, gt_features, **kwargs):
        ...
        rings, ring_cls_targets, ring_reg_targets = self.sample_rings(
            polygons, gt_features, ring_len=self.ring_sample_conf['length']
        )
        batch_sizes = [len(x) for x in rings]

        rings = torch.cat(rings, dim=0).to(img.device)
        ring_cls_targets = torch.cat(ring_cls_targets, dim=0).to(img.device)
        ring_reg_targets = torch.cat(ring_reg_targets, dim=0).to(img.device)

        norm_rings, offset_rings = self.normalize_rings(
            rings, self.ring_sample_conf['interval'] * self.ring_sample_conf['length'], 'global'
        )
        norm_ring_reg_targets, offset_ring_reg_targets = self.normalize_rings(
            ring_reg_targets, self.ring_sample_conf['interval'] * self.ring_sample_conf['length'], 'global'
        )
        ...
        for size in batch_sizes:
            ...
            for ring, gt_ring, cur_next_scores in zip(temp, ring_reg_targets[start_idx:start_idx+size].cpu(), temp2):
                cur_rings.append(ring[ring >= 0].view(-1, 2))
                cur_gt_rings.append(gt_ring[gt_ring >= 0].view(-1, 2))
                ...
                cur_next_rings.append(ring[temp])
    ...
    def decode_ring_next(self, next_idxes, valid_mask=None):
        x = 0
        pred_idxes = []
        ...
        if next_idxes[x] > x and (valid_mask is None or valid_mask[x]):
            pred_idxes.append(x)
    ...
    def slide_inference(self, img, img_meta, rescale, contours=None, contour_labels=None,
        ...
        for i in range(len(all_ring_sizes)):
            ...
            for j in range(len(all_ring_sizes[i])):
                ...
                if self.test_cfg.get('use_ring_next', False):
                    pred_idxes = self.decode_ring_next(ring_next.max(dim=1)[1])
