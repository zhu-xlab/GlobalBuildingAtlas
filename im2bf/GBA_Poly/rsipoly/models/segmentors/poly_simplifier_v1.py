...
class PolySimplifierV1(BaseModule):
    ...
    def __init__(
        self, backbone, assigner, sampler, wandb_cfg=None,
        window_feat_size=6, len_sampled_segments=50, init_cfg=None,
        train_cfg=None, test_cfg=None, **kwargs,
    ):
        super(PolySimplifierV1, self).__init__(init_cfg)
    ...
    def prepare_node_feats(self, N, batch_idxes, batch_sizes, segments, labels, window_feats, W):
        ...
        attn_mask = attn_mask.unsqueeze(1).repeat(1, self.backbone.num_heads, 1, 1).view(-1, N, N)

        return node_feats, attn_mask, batch_labels

    def forward_train(self, img, img_metas, contours, contour_labels, gdal_features, gt_features, **kwargs):
        ...
        node_preds = self.backbone(node_feats, attn_mask)
