...
class RegularEncoderDecoder(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(RegularEncoderDecoder, self).__init__(init_cfg)
        if pretrained is not None:
            assert backbone.get('pretrained') is None, \
                'both backbone and segmentor set pretrained weight'
            backbone.pretrained = pretrained
        self.backbone = builder.build_backbone(backbone)
        self.crf_head = builder.build_head(crf_head)
        if neck is not None:
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        states.update({'img': img,
                       'gt': gt})
        probs = F.softmax(states['seg_logits'], dim=1).cpu()
        probs = F.interpolate(probs, size=img.shape[2:])
        states['vis|masks_preds'] = [img[:, :3].cpu(), probs.max(dim=1)[1]]
        states['vis|masks_gt'] = [img[:, :3].cpu(), (gt > 0).byte().squeeze(1).cpu()]
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        loss_decode, state_decode = self._decode_head_forward_train(x, img_metas, gt_semantic_seg)
        losses.update(loss_decode)
        states.update(state_decode)

        seg_logits = states['seg_logits']
        probs = F.softmax(seg_logits, dim=1)
        mean, std, _ = img_metas[0]['img_norm_cfg'].values()
        loss_crf = self.crf_head(img[:, :3].clone(), probs, mean=mean[:3], std=std[:3])
