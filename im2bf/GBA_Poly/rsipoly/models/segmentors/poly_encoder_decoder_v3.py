...
class PolyEncoderDecoderV3(EncoderDecoder):
    ...
    def __init__(self, backbone, decode_head, point_det_head, matching_head, suppression,
                 wandb_cfg=None, post_processor=None, **kwargs):
        super(PolyEncoderDecoderV3, self).__init__(backbone, decode_head, **kwargs)
        ...
        if post_processor:
            self.post_processor = builder.build_head(post_processor)
    ...
    def forward_train(self, img, img_metas, contours, gt_points, polygons, mask, **kwargs):
        ...
        proposals = self.get_proposals(contours, H, W, num_min_proposals=512, device=img.device)
    ...
    def get_graph_targets(self, pos_inds, gt_inds, gt_points, polygons_list, num_nodes=512):
    ...
    def get_polygons(self, img, contours):
        ...
        bbox_preds = self.generate_bbox_from_point(point_preds_ori[0], width=7)
        ...
        if self.post_processor is not None:
            self.post_processor.post_process(img, polygons)
