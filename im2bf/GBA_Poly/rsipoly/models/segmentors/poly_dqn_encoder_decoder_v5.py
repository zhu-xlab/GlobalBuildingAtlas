...
class PolyDQNEncoderDecoderV5(EncoderDecoder):
    ...
    def __init__(self, backbone, decode_head, assigner, sampler, dqn_head, point_head, wandb_cfg=None,
                 ...
                 dqn_skill_level=4, matching_head=None, evaluate_freq=50, **kwargs):
        super(PolyDQNEncoderDecoderV5, self).__init__(backbone, decode_head, **kwargs)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if not self.freeze_base_net:
            ...
            if init_success:
                games = self.dqn_head.sample_games(game_level)

                success_vector = None
                success_rate = 0.
                for i in range(game_level * 2):
                ...
                if self.cur_iter % self.evaluate_freq == 0:
                    success_rate_act = self.dqn_head.evaluate(games, net_type='act')
                    success_rate_target = self.dqn_head.evaluate(games, net_type='target')
                    if success_rate_act > success_rate_target:
                        self.dqn_head.update_target_net()

                    if success_rate_act >= 0.8 and len(games) >= 10:
