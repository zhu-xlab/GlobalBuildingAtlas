...
class PolyDQNEncoderDecoderV10(EncoderDecoder):
    ...
    def __init__(self, backbone, decode_head, assigner, sampler, dqn_head, point_head, wandb_cfg=None,
        ...
        super(PolyDQNEncoderDecoderV10, self).__init__(backbone, decode_head, **kwargs)
            ...
            if games is not None:
                ...
                if self.evaluate_freq > 0 and self.cur_iter % self.evaluate_freq == 0:
                    ...
                    for level in levels:
                        ...
                        scores.append(1 if rate1 > rate2 else 0)
                        print(f'level: {level}, act_net: {rate1}, target_net: {rate2}')

                    if np.array(scores).mean() > 0.5:
    ...
    def get_window_feats(self, img, points, w):
        N, _ = points.shape
        offset = torch.cartesian_prod(torch.arange(w), torch.arange(w)).to(points.device)
        mask = torch.where(img[0, 0] > 0, 1, 0)
        padded_mask = F.pad(mask, (w, w, w, w), mode='constant', value=0)

        windows = points.unsqueeze(1) + offset.unsqueeze(0)
        ...
        for comp_idx in comp_idxes:
            ...
            if len(cur_inds) > 0 and ~(next_inds == -1).any() and \
               ...
               (cur_inds.unique() == next_inds.unique()).all(): # valid components
                ...
                # print(comp_idx)
        pdb.set_trace()
        comp_point_window_feats = [self.get_window_feats(img, x, self.window_feat_size) for x in comp_points]
        if for_dqn:
            return dict(
