...
class PolySimplifierV12(BaseModule):
    ...
    def __init__(
        ...
        super(PolySimplifierV12, self).__init__()
    ...
    def sample_rings(self, pred_polygon, gt_polygon):
        ...
        return sampled_ring, sampled_cls_target, sampled_reg_target
    ...
    def forward_train(self, img, seg_logits, gt_semantic_seg, gt_features, **kwargs):
        ...
        for cur_pred_mask, cur_gt in zip(up_pred_masks.cpu(), up_gt.cpu()):
            # assigned_pred_mask = self.assign_gt_pred(cur_pred_mask, cur_gt)
        ...
        batch_matched_rings = self.get_matched_rings(pred_polygons, gt_features)
        ...
        rings, norm_rings, ring_cls_targets, ring_reg_targets, ring_probs = [], [], [], [], []
        for i, matched_rings in enumerate(batch_matched_rings):
            ...
            for pred_ring, gt_ring in matched_rings:
                ring, ring_cls_target, ring_reg_target = self.sample_rings(pred_ring, gt_ring)
            ...
            if len(cur_rings) > 0:
                ...
                ring_prob = self.get_ring_probs(norm_ring, probs[i:i+1])
    ...
    # TODO refactor
    def slide_inference(self, img, probs, img_meta, rescale=None, contours=None, contour_labels=None,
        ...
        poly_json = self.polygonize(img[:,0])[0]
        # polygons = [x['coordinates'] for key in poly_json[0].keys() for x in poly_json[0][key]]
        # polygons = [x['coordinates'] for key in poly_json[0].keys() for x in poly_json[0][key]]

        all_rings, all_idxes, all_ring_sizes = polygon_utils.sample_rings(poly_json)
        ...
        """
        temp = torch.zeros(N, int(max_sample_length), int(max_sample_length), 32)
        pos_encoding = self.pos_enc.cpu()(temp).permute(0,3,1,2)
        point_feats = F.grid_sample(
            pos_encoding,
            norm_rings.view(N, 1, L, 2),
            align_corners=True
        ).squeeze(2).permute(0,2,1) # (B, num_points, num_channels)

        rings_input = torch.cat([norm_rings, seq, point_feats], dim=-1)
        """
        ring_probs = self.get_ring_probs(norm_rings, probs)
        ...
        ring_pred_reg = []
        ring_pred_next = []
        # for i in tqdm(range(num_iter), desc=f'Simplifying the polygons on {img_meta[0]["filename"]}'):
        ...
        pred_polygons = self.polygon_processor.post_process_without_format(pred_rings, ring_pred_next, all_idxes, all_ring_sizes)
        # self.polygon_processor_cython.post_process(pred_rings, ring_pred_next, all_idxes, all_ring_sizes, img_meta[0])

        new_pred_polygons = []
        for rings in pred_polygons:
            new_rings = [ring.numpy().tolist() for ring in rings if len(ring) >= 3]
            if len(new_rings) > 0:
                new_pred_polygons.append(new_rings)

        return new_pred_polygons
    ...
    def get_ring_probs(self, norm_ring, probs, type='grid_sample'):
        ...
        if type == 'grid_sample':
            ring_prob = F.grid_sample(
                probs, norm_ring.view(1, 1, num_rings * ring_len, 2).to(probs.device),
                align_corners=True
            ).squeeze(2).permute(0,2,1) # (B, num_points, num_channels)
