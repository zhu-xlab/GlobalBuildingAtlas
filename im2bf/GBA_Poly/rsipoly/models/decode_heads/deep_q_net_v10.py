...
class DeepQNetV10(BaseModule):
    ...
    def __init__(
        ...
        super(DeepQNetV10, self).__init__()
    ...
    def evaluate(self, games, net_type='act'):
        ...
        for i in range(N * 2):
            ...
            scores = torch.zeros(B, N, 2, device=points.device)
            with torch.no_grad():
        ...
        for key, value in ious.items():
            success_rate[key] = (np.array(value) >= self.pass_reward_thre).mean()
    ...
    def forward_train(self, games):
        ...
        if self.add_gt_state:
            ...
            loss_gt_state = dict(
                loss_gt_state=torch.zeros(1, device=games['points'][0].device),
            )
    ...
    def cal_loss_gt_state(self, games):
        ...
        gt_scores = torch.zeros(B, N, dtype=torch.long, device=scores.device)
        attn_mask = torch.zeros(B, N, dtype=torch.bool, device=scores.device)
        ...
        pdb.set_trace()
        # pos_mask = (gt_scores[attn_mask] == 1).view(-1)
        loss_gt_state = self.loss_fun(
            scores.view(-1, 2), gt_scores.view(-1)
        )[attn_mask.view(-1)]
        loss_gt_state = loss_gt_state.mean()
