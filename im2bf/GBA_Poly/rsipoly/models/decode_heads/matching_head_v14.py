...
class OptimalMatchingV14(BaseModule):
    ...
    def __init__(self, descriptor_dim=64, in_channels=36, in_dim_angle=64, remove_last=False, return_polygons=False, mask_graph=True,
                 ...
                 loss_angle_weight=1., permute_loss_weight=1.0, next_point_loss_weight=20.0,
                 point_cls_loss_weight=0.,
                 num_attention_layers=4):
        super(OptimalMatchingV14, self).__init__()
        ...
        self.fc_gnn_cls = build_linear_layer(
            dict(type='Linear'),
            # in_features=in_channels + 0 if pos_enc_dim < 0 else pos_enc_dim,
            in_features=self.descriptor_dim,
            out_features=2)
    ...
    def forward_train(self, point_feats, graph_targets, point_preds, point_preds_ori, comp_mask=None, probs=None):
        ...
        loss_gnn_cls = self.loss_fun(gnn_cls_preds.view(-1,2), gnn_cls_labels.view(-1))
        loss_gnn_cls = loss_gnn_cls[node_valid_mask.view(-1)].sum() / (node_valid_mask.sum() + 1e-8)
        losses['gnn_cls_loss'] = loss_gnn_cls
        ...
        if self.return_polygons:
            ...
            probs = scores_1 + scores_2.transpose(1, 2)
    ...
    def forward_test_global(self, point_feats, point_preds, point_preds_ori, comp_mask, **kwargs):
        ...
        for i, score in enumerate(scores):
            N = score.shape[0]
            sorted_idxes = score.view(-1).sort(descending=True)
            first_row, first_col = sorted_idxes[1][0] // N, sorted_idxes[1][0] % N
            pdb.set_trace()
