...
class DeepQNetV8(BaseModule):
    ...
    def __init__(
        ...
        super(DeepQNetV8, self).__init__()
    ...
    def get_next_states(self, point_seqs, actions, points_ori, gt_polygons, gt_poly_idxes_list):
        ...
        T = point_seqs.size(1)
        seq_idxes = (point_seqs > 0).nonzero()
        if len(seq_idxes) > 0 and point_seqs[:,1].max() == N:
            pdb.set_trace() # invalid case

        point_seq_feats = torch.zeros(B, T, N, device=point_seqs.device, dtype=torch.int)
        point_seq_feats[seq_idxes[:,0], seq_idxes[:,1], point_seqs[seq_idxes[:,0], seq_idxes[:,1]]] = 1
        degrees = point_seq_feats.sum(dim=1)

        # TODO: terminate when actions == -1
        # terminals = (point_seqs == actions.unsqueeze(1)).any(dim=1)
        # terminals = terminals | ((point_seqs >= 0).sum(dim=1) >= N)
        terminals = actions >= N
        active_mask = actions < N
        # active_mask = actions >= 0

        cur_poly_end_idx = (point_seqs >= 0).sum(dim=1)
        next_point_seqs = point_seqs.clone()
        next_point_seqs[
            torch.arange(B, device=point_seqs.device)[active_mask],
            cur_poly_end_idx[active_mask]
        ] = actions[active_mask]

        rewards = torch.zeros(B, device=point_seqs.device)
        log_vars = [{'pass_exam': False} for _ in range(B)]
    ...
    def evaluate(self, games, net_type='act'):
        ...
        B, N, _ = point_preds.shape
        active_games = torch.ones(B, device=point_preds.device, dtype=torch.bool)
        cur_point_seqs = point_seqs.clone()
        ...
        for i in range(N * 2):
            ...
            with torch.no_grad():
                scores = self.forward_gnn(
                    point_preds[active_games],
                    point_feats[active_games],
                    cur_point_seqs[active_games],
                    type=net_type
                )
            ...
            next_point_seqs, rewards, terminals, log_vars = self.get_next_states(
                cur_point_seqs, actions, point_preds_ori, gt_polygons, gt_poly_idxes_list
            )
    ...
    def sample_games(self, game_level):
    ...
    def forward_train(self, games):
        ...
        if self.loss_dqn_weight > 0:
            ...
            loss_dqn = dict(
                loss_dqn = torch.zeros(1, device=games[0]['point_preds'].device),
                loss_terminal = torch.zeros(1, device=games[0]['point_preds'].device),
            )
        ...
        if self.add_gt_state:
            ...
            loss_gt_state = torch.zeros(1, device=games[0]['point_preds'].device)
        losses['loss_gt_state'] = loss_gt_state
    ...
    def cal_loss_gt_state(self, games):
        ...
        # print(gt_point_seqs[:10])

        scores = self.forward_gnn(point_preds, point_feats, gt_point_seqs, type='act')
        mask = actions >= 0
        loss_gt_state = self.cse_loss_fun(scores[mask], actions[mask])
        return loss_gt_state
