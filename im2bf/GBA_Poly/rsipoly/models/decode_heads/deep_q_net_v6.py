...
class DeepQNetV6(BaseModule):
    ...
    def __init__(
        ...
        super(DeepQNetV6, self).__init__()
    ...
    def forward_gnn(self, point_preds, point_feats, state, type='act'):
        ...
        state_feats = torch.cat([state_one_hot, points], dim=1).view(B, 5, N*N)
        ...
        """
        if type == 'act':
            scores, = self.act_net(gnn_feats.view(B, -1, N, N))
        else:
            scores, = self.target_net(gnn_feats.view(B, -1, N, N))
        ...
        """
        if type == 'act':
            scores, offsets = self.act_net(gnn_feats)
        else:
            scores, offsets = self.target_net(gnn_feats)
    ...
    def sample_actions(self, scores, states, method='random_positive', random_eps=0.1):
        B, N, _ = scores.shape
        mask = scores > 0
        actions = []
        for i in range(B):
    ...
    def get_next_state(self, cur_state, action, points_ori, gt_polygons, gt_edge):
        ...
        next_state[action[1], action[0]] += 1
        degrees = next_state.sum(dim=1)

        if (degrees[action] > 2).any():
            self.cnt_invalid_0 += 1
            return self.get_first_state(cur_state, gt_edge), self.reward_invalid_0, True, {}

        # check validity
        # if (next_state.sum(dim=0) > 2).any() or (next_state.sum(dim=1) > 2).any():
        #     self.cnt_invalid_0 += 1
        ...
        poly_idxes_list = self.get_connected_components(next_state)

        # collect polygons
        valid_polygons = []
        return_polygons = []
        has_invalid = False
        for components in poly_idxes_list:
            if (degrees[components] == 2).all():
                if len(components) >= 4:
                    cur_polygon = points_ori[components]
                ...
                elif len(components) > 1 and len(components) <= 3:
        ...
        if has_invalid:
            return self.get_first_state(cur_state, gt_edge), reward, True, {}
    ...
    def forward_train(self, game_level, exam_mode=False):
        ...
        scores = self.forward_gnn(point_preds, point_feats, states, type='act')
        actions = self.sample_actions(scores, states, method=self.action_sample_type)
        actions = torch.stack(actions, dim=0)
        next_states, rewards, terminals, log_vars = [], [], [], []
        ...
        next_states = torch.stack(next_states, dim=0)
        next_scores = self.forward_gnn(point_preds, point_feats, next_states, type='target')
        ...
        #     pdb.set_trace()
        terminal_mask = torch.tensor(terminals, device=scores.device)
        pos_mask = Y_batch > 0
        ...
        loss_terminal = loss_dqn[terminal_mask].sum() / (terminal_mask.sum() + 1e-8)
        loss_dqn = loss_dqn[~terminal_mask].sum() / ((~terminal_mask).sum() + 1e-8)
