...
class OptimalMatchingV4(BaseModule):

    def __init__(self, descriptor_dim=64, remove_last=False, return_polygons=False):
        super(OptimalMatchingV4, self).__init__()
    ...
    def forward_train(self, img, feats, graph_targets, point_preds, probs=None):

        node_feat = F.grid_sample(feats, point_preds.unsqueeze(1), align_corners=True)
    ...
    def forward_test(self, img, point_preds):
        ...
        scores = scores_to_permutations(scores)
        polygon = permutations_to_polygons(scores, point_preds, out='numpy')[0]

        return [polygon], [point_pred for point_pred in point_preds]
