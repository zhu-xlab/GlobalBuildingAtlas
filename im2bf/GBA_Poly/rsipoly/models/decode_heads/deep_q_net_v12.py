...
class DeepQNetV12(BaseModule):
    ...
    def __init__(
        ...
        num_max_points=32, max_batch_size=256, max_base_size=80, random_merge_prob=0.0
    ):
        super(DeepQNetV12, self).__init__()
        ...
        self.random_merge_prob = random_merge_prob
    ...
    def gen_gt_point_seqs(self, gt_poly_idxes_list, batch_idxes, batch_sizes, device):
        ...
        for i in range(B):
            ...
            if random.random() < self.random_merge_prob:
                cur_gt_point_seqs = torch.zeros(self.num_max_points, device=device, dtype=torch.long) - 1
                sizes = [x for x in batch_sizes[i]]
                N = sum(sizes) + 1
                merged_gt_poly_idxes = []
                start_idx = 0
                for j, idx in enumerate(batch_idxes[i]):
                    temp = [(np.array(x) + start_idx).tolist() for x in gt_poly_idxes_list[idx]]
                    merged_gt_poly_idxes += temp
                    start_idx += batch_sizes[i][j]

                shuffled_idxes = [cycle_shuffle(x[:-1]) for x in merged_gt_poly_idxes]
    ...
    def cal_loss_gt_state(self, games):
        ...
        scores = self.forward_gnn(points, point_feats, gt_point_seqs, new_batch_sizes, type='act')
        ...
        for i, actions in enumerate(batch_actions):
            ...
            for j, action in enumerate(actions):
                ...
                # start_idx += batch_sizes[i][j]
                start_idx += new_batch_sizes[i][j] + 1
