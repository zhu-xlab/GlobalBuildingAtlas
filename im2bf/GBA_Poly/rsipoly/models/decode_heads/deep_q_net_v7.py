...
class DeepQNetV7(BaseModule):

    def __init__(
        self, gnn_net=None, in_channels=512, hidden_channels=64, num_gnn_layers=3, return_polygons=False,
        ...
        num_max_points=32, max_batch_size=256
    ):
        super(DeepQNetV7, self).__init__()

        self.act_net = builder.build_backbone(gnn_net)
        self.target_net = builder.build_backbone(gnn_net)
    ...
    def forward_gnn(self, point_preds, point_feats, point_seqs, type='act'):
        ...
        # state_one_hot = state.unsqueeze(1)
        point_seq_feats = torch.zeros(B, T, N, device=point_preds.device, dtype=torch.int)
        seq_idxes = (point_seqs > 0).nonzero()
        assert len(seq_idxes) == 0 or seq_idxes[:,1].max() < N
        ...
        if type == 'act':
            scores, _ = self.act_net(node_feats)
        else:
            scores, _ = self.target_net(node_feats)

        scores = scores.view(B, N)
        return scores

    def sample_actions(self, scores, point_seqs, method='random_positive', random_eps=0.1):
        ...
        if method == 'node_based':
            ...
            #j if (cur_poly_end_idx >= 5).any():
            #j     pdb.set_trace()
            # cur_poly_valid_idx = torch.where(cur_poly_start_idx + 4 <= cur_poly_end_idx + 1,
            #                                  cur_poly_start_idx + 4, cur_poly_end_idx + 1)
            ...
            for i in range(B):
                if method == 'random_positive':
                ...
                elif method == 'random_graduate':
                    degrees = states[i].sum(dim=1)
                    valid_idx = (degrees == 1).nonzero().view(-1)
                    if len(valid_idx) > 0:
                        scores_mask = torch.zeros_like(scores[i], dtype=torch.uint8)
                        scores_mask[degrees == 1] = 1
                        scores_mask[:, degrees == 1] = 1
                        pos_scores = torch.where(scores_mask, scores[i], -1e8)
                        cur_action = pos_scores.argmax().item()
                        cur_action = torch.tensor([cur_action // N, cur_action % N], device=scores.device)
                    else:
                        cur_action = scores[i].argmax().item()
                        cur_action = torch.tensor([cur_action // N, cur_action % N], device=scores.device)
                    ...
                    if len(valid_idx) > 0:
                        ...
                        scores_mask = torch.ones_like(scores[i], dtype=torch.uint8)

                    idxes = scores_mask.nonzero()
                    same_idxes = idxes[idxes[:, 0] == idxes[:, 1]]
                    diff_idxes = idxes[idxes[:, 0] != idxes[:, 1]]
                    same_invalid_mask = (degrees[same_idxes] != 0).any(dim=1)
                    diff_invalid_mask = (degrees[diff_idxes] == 2).any(dim=1)
                    same_invalid_idxes = same_idxes[same_invalid_mask]
                    diff_invalid_idxes = diff_idxes[diff_invalid_mask]
                    scores_mask[same_invalid_idxes[:, 0], same_invalid_idxes[:, 1]] = 0
                    scores_mask[diff_invalid_idxes[:, 0], diff_invalid_idxes[:, 1]] = 0

                    if (states[i] == 0).all():
                        scores_mask[range(N), range(N)] = 0
                    ...
                    if pos_mask.sum() > 0:
                        if random.random() < random_eps:
                            cur_action = random.choice(pos_mask.nonzero())
                        else:
                            cur_action = pos_scores.argmax().item()
                            cur_action = torch.tensor([cur_action // N, cur_action % N], device=scores.device)
                    else:
                ...
                if cur_action[0] > cur_action[1]:
                    cur_action = cur_action[[1,0]]
    ...
    def get_next_states(self, point_seqs, actions, points_ori, gt_polygons, gt_poly_idxes_list):
        ...
        point_seq_feats[seq_idxes[:,0], seq_idxes[:,1], point_seqs[seq_idxes[:,0], seq_idxes[:,1]]] = 1
        degrees = point_seq_feats.sum(dim=1)
        terminals = (point_seqs == actions.unsqueeze(1)).any(dim=1)
        terminals = terminals | ((point_seqs >= 0).sum(dim=1) >= N-1)


        cur_poly_end_idx = (point_seqs >= 0).sum(dim=1)
        next_point_seqs = point_seqs.clone()
        next_point_seqs[torch.arange(B, device=point_seqs.device), cur_poly_end_idx] = actions

        rewards = torch.zeros(B, device=point_seqs.device)
        log_vars = [{} for _ in range(B)]

        for batch_idx in terminals.nonzero().view(-1):
            poly_idxes_list = next_point_seqs[batch_idx][next_point_seqs[batch_idx] >= 0]
            components = []
            start_idx = 0
            for idx in range(1, len(poly_idxes_list)):
                if idx > start_idx and poly_idxes_list[start_idx] == poly_idxes_list[idx]:
                    components.append(poly_idxes_list[start_idx:idx].tolist())
                    start_idx = idx + 1
            ...
            for component in components:
                ...
                elif len(component) > 1 and len(component) <= 3:
            ...
            if has_invalid:
                next_point_seqs[batch_idx] = self.get_first_state(next_point_seqs[batch_idx],
                                                                  gt_poly_idxes_list[batch_idx])
                rewards[batch_idx] = self.reward_invalid_2

            else:
                if len(valid_polygons) > 0:
                    self.cnt_valid += 1
                    iou_mat = np.zeros((len(valid_polygons), len(gt_polygons[batch_idx])))
                    for i, pred_poly in enumerate(valid_polygons):
                        for j, gt_poly in enumerate(gt_polygons[batch_idx]):
                            try:
                                with warnings.catch_warnings(record=True) as w:
                                    iou_mat[i, j] = self.cal_iou(pred_poly, gt_poly)
                                    # if self.cnt_valid >= 50:
                                    #     pdb.set_trace()
                                    if w:
                                        pass
                            except Exception:
                    ...
                    reward = iou_mat[r, c].sum() / len(gt_polygons[batch_idx]) * 1
                    # if len(gt_polygons) > 1:
                    #     pdb.set_trace()

                    if len(valid_polygons) > len(gt_polygons[batch_idx]):
                        reward -= (len(valid_polygons) - len(gt_polygons[batch_idx])) * self.extra_poly_penalty

                    self.iou_scores.append(reward)

                    states = {'pass_exam': reward > self.pass_reward_thre,
                              'return_polygons': return_polygons}
                    log_vars[batch_idx] = states

                    # TODO: determine pass or not based on the iou of two polygon sets
                    next_point_seqs[batch_idx] = self.get_first_state(next_point_seqs[batch_idx],
                                                                      gt_poly_idxes_list[batch_idx])
                    rewards[batch_idx] = reward
                    # return self.get_first_state(cur_state, gt_edge), reward * 10000, True, states
                else:
                    self.cnt_valid_2 += 1
                    next_point_seqs[batch_idx] = self.get_first_state(next_point_seqs[batch_idx],
                                                                      gt_poly_idxes_list[batch_idx])
                    rewards[batch_idx] = self.reward_valid2

        return next_point_seqs, rewards, terminals, log_vars

    def evaluate(self, game_level, net_type='act'):
        self.init_state(game_level)
        ...
        success_vector = np.zeros(len(games), dtype=np.int)
        for i in range(game_level):
            with torch.no_grad():
                scores = self.forward_gnn(point_preds, point_feats, states, type=net_type)
                actions = self.sample_actions(scores, states, method='one_by_one', random_eps=-1)
            ...
            for i, game in enumerate(games):
                ...
                states[i] = next_state
                passes.append(log_var.get('pass_exam', False))

            success_vector = success_vector | np.array(passes)
    ...
    def forward_train(self, game_level, exam_mode=False):
        ...
        return {
            'loss_dqn': loss_dqn * self.loss_dqn_weight,
            'loss_terminal': loss_terminal * self.terminal_weight,
            # 'loss_row_col': loss_row_col * self.loss_row_col_weight,
            'loss_gt_state': loss_gt_state * self.loss_gt_state_weight
        }, log_dict

    def gen_gt_point_seqs(self, gt_poly_idxes_list, point_preds):
        ...
        gt_point_seqs = torch.zeros(B, self.num_max_points, device=point_preds.device, dtype=torch.long) - 1
        actions = torch.zeros(B, device=point_preds.device, dtype=torch.long)
        for i in range(B):
            gt_poly_idxes = gt_poly_idxes_list[i]
            ...
            shuffled_idxes = [torch.tensor(x, device=point_preds.device) for x in shuffled_idxes]
            shuffled_idxes = torch.cat(shuffled_idxes, dim=0)
            assert len(shuffled_idxes) > 0

            len_seq = random.randint(0, len(shuffled_idxes)-1)
            gt_point_seqs[i, :len_seq] = shuffled_idxes[:len_seq]
            actions[i] = shuffled_idxes[len_seq]

        return gt_point_seqs, actions
    ...
    def cal_loss_gt_state(self, games):
        ...
        gt_poly_idxes_list = [game['poly_idxes_list'] for game in games]
        B, N, _ = point_preds.shape
        gt_point_seqs, actions = self.gen_gt_point_seqs(gt_poly_idxes_list, point_preds)

        scores = self.forward_gnn(point_preds, point_feats, gt_point_seqs, type='act')
        ...
        Q_batch = torch.gather(scores, 1, actions.unsqueeze(1)).view(-1)
        Y_batch = torch.ones_like(Q_batch)
        # Y_batch[~terminals] = rewards[~terminals] + next_scores.max(dim=1)[0][~terminals] * self.gamma
        # Y_batch[terminals] = rewards[terminals]
        loss = self.loss_fun(Q_batch, Y_batch).mean()
