...
class OptimalMatchingV7(BaseModule):

    def __init__(self, descriptor_dim=64, remove_last=False, return_polygons=False, mask_graph=True,
                 block_comp_mask=False, out_dim_angle=36):
        super(OptimalMatchingV7, self).__init__()
        ...
        self.gnn = AttentionalGNN(self.descriptor_dim, self.attention_layers, first_dim=out_dim_angle)

        self.fc_angle = build_linear_layer(
            dict(type='Linear'),
            # in_features=in_channels + 0 if pos_enc_dim < 0 else pos_enc_dim,
            in_features=descriptor_dim,
    ...
    def forward_train(self, img, point_feats, graph_targets, point_preds, gt_angles, comp_mask=None, probs=None):
        losses = {}
        if self.remove_last:
        ...
        loss_angle = self.loss_fun(point_angles.view(B*N, -1),
                                   gt_angles.argmax(dim=-1).view(B*N))[angle_mask].sum() / (angle_mask.sum() + 1e-8)
        losses['angle_loss'] = loss_angle
        ...
        if comp_mask is not None and not self.block_comp_mask:
            # score_mask = torch.where(comp_mask==1, 1, -torch.inf)
            scores_1 = torch.where(comp_mask==1, scores_1, -torch.inf)
            scores_2 = torch.where(comp_mask==1, scores_2, -torch.inf)

        # scores_1 = scores_1 * score_mask
        # scores_2 = scores_2 * score_mask
        ...
        losses['permute_loss'] = (loss_gnn_1 + loss_gnn_2) / 2
