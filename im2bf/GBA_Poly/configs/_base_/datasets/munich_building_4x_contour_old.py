...
train_pipeline = [
    ...
    dict(type='CreatePolyAnn', num_max_vertices=512, add_contour=True),
    # dict(type='RandomFlip', prob=0.5),
    # dict(type='PhotoMetricDistortion'),
    dict(type='Normalize', **img_norm_cfg),
    # dict(type='Pad', size=crop_size, pad_val=0, seg_pad_val=255),
    dict(type='PolyFormatBundle'),
    dict(type='Collect', keys=['img', 'vertices', 'vertices_mask', 'permute_matrix', 'mask']),
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(
        ...
        transforms=[
            ...
            dict(type='CreateVertices'),
            dict(type='Normalize', **img_norm_cfg),
            dict(type='ImageToTensor', keys=['img']),
            dict(type='Collect', keys=['img', 'corner_points']),
        ])
]
